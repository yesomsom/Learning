...
public class MemberDAO {
   ...
   public int updateMember(MemberVO memberVO) {
      sqlMapper = getInstance();
      SqlSession session = sqlMapper.openSession();      
      int result = session.insert("mapper.member.updateMember", memberVO);
      session.commit();
      return result;
   }
    ...
    * getInstance(); SqlSession session = sqlMapper.openSession(); List list =
    * session.selectList("mapper.member.searchMember", memberVO); return list; }
   ...
   public List<MemberVO> selectLike(String name){
      sqlMapper = getInstance();
      SqlSession session = sqlMapper.openSession();
      List list = session.selectList("mapper.member.selectLike", name); //selectLike
      return list;
   }
}