...
<h3>sms인증페이지</h3>
<div id="contents">
<form action="${path }/page/check/sendSms/insert.do" method="post">
  ...
  <input type="button" value="닫기" onClick="setParaentText();">
</form>  
    </div>
    
<script type="text/javascript">
	function setParaentText(){
		opener.document.getElementById("phoneNum_all").value = document.getElementById("to").value
...
<!-- 휴대폰 번호 유효성 검사 -->
  <script>
function Phonecheck() {
   var objPhoneNum = document.getElementById("phoneNum_all");
    ...
    $("#send").click(function() {
       ...
       var con_test = confirm("해당번호로 인증문자를 발송하시겠습니까?");  
          ...
          if(con_test == true){
        	  // 인증번호 얻기위한 통신
        	  $.ajax({
        		  url : "/page/check/sendSms/insert.do",
        		  type : "post",
        		  data:{
        			to : $("#to").val(),
        			text : $("#text").val()
